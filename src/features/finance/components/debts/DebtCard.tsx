  ...
  return (
    <div className={`settings-card p-5 group ${debt.is_paid_off ? 'opacity-60' : ''}`}>
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            {debt.is_paid_off && (
              <span className="text-[9px] font-mono bg-green-400/10 text-green-400 border border-green-400/20 px-1.5 py-0.5 rounded uppercase">Paid Off</span>
            )}
          </div>
        </div>
      ...
      <div className="space-y-2 mb-4 text-xs font-mono">
        ...
        <div className="flex justify-between">
          <span className="text-terminal-muted">Original</span>
          <span className="text-white">{formatCurrency(debt.original_amount, currency)}</span>
        </div>
        {debt.interest_rate > 0 && (
          <div className="flex justify-between">
            <span className="text-terminal-muted">Interest Rate</span>
            <span className="text-yellow-400">{debt.interest_rate}% / yr</span>
          </div>
        )}
        {debt.minimum_payment && (
          <div className="flex justify-between">
            <span className="text-terminal-muted">Min. Payment</span>
            <span className="text-white">{formatCurrency(debt.minimum_payment, currency)}</span>
          </div>
        )}
        {debt.due_day && (
          <div className="flex justify-between">
            <span className="text-terminal-muted">Due Day</span>
            <span className="text-white">Day {debt.due_day}</span>
          </div>
        )}
      </div>
      ...
      <p className="text-[9px] font-mono text-terminal-muted mb-3">{pctPaid.toFixed(0)}% paid off</p>
