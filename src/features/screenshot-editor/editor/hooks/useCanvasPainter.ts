...
export const useCanvasPainter = ({
  ...
  const draw = useCallback(() => {
    ...
    const renderOverlay = (overlay: OverlayImage) => {
      ...
      const safeCrop = overlay.crop
        ? {
            x: Math.max(0, Math.min(img.naturalWidth - 1, Math.round(overlay.crop.x))),
            y: Math.max(0, Math.min(img.naturalHeight - 1, Math.round(overlay.crop.y))),
            width: Math.max(1, Math.round(overlay.crop.width)),
            height: Math.max(1, Math.round(overlay.crop.height)),
          }
        : null;

      const sourceX = safeCrop?.x ?? 0;
      const sourceY = safeCrop?.y ?? 0;
      const sourceW = safeCrop ? Math.max(1, Math.min(safeCrop.width, img.naturalWidth - sourceX)) : img.naturalWidth;
      const sourceH = safeCrop ? Math.max(1, Math.min(safeCrop.height, img.naturalHeight - sourceY)) : img.naturalHeight;

      // Use crop dimensions for display size so the overlay shrinks to the cropped region
      const drawW = (safeCrop ? sourceW : overlay.width) * overlay.scale;
      const drawH = (safeCrop ? sourceH : overlay.height) * overlay.scale;
