        ...
        try {
            ...
            const chatMessages: Message[] = Array.isArray(messagesRows)
                ? messagesRows.map((row) => {
                    ...
                    const role = row.user_id === ticketRow.claimed_by || row.user_id === ticketRow.closed_by
                        ? UserRole.ADMIN
                        : UserRole.USER;
                    return {
                        id: row.id,
            ...
            const latestChatTime = chatMessages.length
            ...
            if (ticketRow.closed_at && ticketRow.closed_by_name) {
                ...
                const anchorTime = Math.max(
                    createdTime ?? closedTimeRaw,
                    latestChatTime ? latestChatTime + 1000 : closedTimeRaw
                );
            ...
            const mergedMessages = [...chatMessages, ...logMessages].sort((a, b) => {
  ...
  return (
    <div className="flex flex-col h-full bg-terminal-dark animate-fade-in-up">
      ...
      <div className="h-14 border-b border-terminal-border bg-terminal-panel px-6 flex items-center">
        <button 
            onClick={onBack}
            className="flex items-center gap-2 text-sm text-terminal-muted hover:text-white transition-colors"
        >
            <ArrowLeft size={16} />
      ...
      <div className="flex-1 overflow-hidden relative">
        <div className="h-full max-w-5xl mx-auto px-4 sm:px-6 py-6 flex flex-col">
            {!ticketId && (
                <div className="text-terminal-muted text-sm">No ticket selected.</div>
            )}
            {ticketId && !ticket && (
                <div className="text-terminal-muted text-sm">Loading transcript...</div>
            )}
            {ticket && (
                <>
                                ...
                                ticket.notes?.map((note) => (
                                    <div key={note.id} className="border border-terminal-border rounded-md bg-black/30 p-3">
                                        <div className="flex items-center justify-between gap-3 text-xs text-terminal-muted mb-2">
                                            ...
                                            <span title={formatFullDateTime(note.createdAt)}>
